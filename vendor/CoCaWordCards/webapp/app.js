...
const DEFAULTS = {
  ...
  fontScaleEn: 3.0, fontScaleZh: 1.0, fontScalePhon: 0.7,
...
if (!_loaded.fontUnified202607) {
  settings.fontScaleEn = 3.0; settings.fontScaleZh = 1.0; settings.fontScalePhon = 0.7;
...
function applyHdrOffsets() {
  for (let i = 0; i < HDR_COUNT; i++) {
    ...
    if (el) el.style.transform = `translate(${settings.hdrX[i]}px, ${settings.hdrY[i]}px)`;
  }
}
...
function render() {
  ...
  els.mods[5].style.visibility = settings.showPlay ? "visible" : "hidden";     // 播放键
...
// The visibility toggles live in the edit bar (not 设置); keep them in sync with settings.
function syncVisToggles() {
  $("showEn").checked = settings.showEn; $("showZh").checked = settings.showZh;
  $("showPos").checked = settings.showPos; $("showPhon").checked = settings.showPhon;
  $("showButtons").checked = settings.showButtons; $("showPlay").checked = settings.showPlay;
  $("showArrows").checked = settings.showArrows;
  $("showPack").checked = settings.showPack; $("showGroup").checked = settings.showGroup;
...
function syncHdrFontSliders() {
  $("fontScaleGroup").value = settings.fontScaleGroup;
  $("fontValGroup").textContent = settings.fontScaleGroup.toFixed(1) + "×";
  $("fontScaleProgress").value = settings.fontScaleProgress;
  $("fontValProgress").textContent = settings.fontScaleProgress.toFixed(1) + "×";
}
...
function bindEditDrag() {
  els.mods.forEach((mod, i) => {
    ...
    mod.addEventListener("pointermove", (e) => {
      ...
      mod.style.transform = `translate(${settings.moduleX[i]}px, ${settings.moduleY[i]}px)`;
    });
    const end = (e) => { if (mod.hasPointerCapture(e.pointerId)) mod.releasePointerCapture(e.pointerId); mod.classList.remove("dragging"); };
    mod.addEventListener("pointerup", end);
    mod.addEventListener("pointercancel", end);
  });
}
// Edit-mode drag for the three header modules (词包/分组/进度). Offsets stored in
// hdrX/hdrY. The pack <select> is made non-interactive during edit (CSS) so the
// container itself receives the drag pointer.
function bindHdrDrag() {
  els.hdrMods.forEach((mod, i) => {
    ...
    const clampX = (v) => {
      ...
      const left = r.left - cur, right = r.right - cur;  // untranslated box edges
    ...
    mod.addEventListener("pointermove", (e) => {
      ...
      mod.style.transform = `translate(${settings.hdrX[i]}px, ${settings.hdrY[i]}px)`;
    });
    const end = (e) => { if (mod.hasPointerCapture(e.pointerId)) mod.releasePointerCapture(e.pointerId); mod.classList.remove("dragging"); };
    mod.addEventListener("pointerup", end);
    mod.addEventListener("pointercancel", end);
...
function openDrawer() {
  ...
  $("drawer").hidden = false; $("drawerMask").hidden = false; syncDrawer();
}
...
function bindEvents() {
  ...
  const visToggle = (id, key) => $(id).addEventListener("change", (e) => { settings[key] = e.target.checked; saveSettings(); render(); });
