...
- **日期必须取真实世界的当天日期**：写日志前先用系统命令获取（如 `date +%Y-%m-%d`），用该真实日期填 `.cl-date`；**严禁沿用上一条目的日期、也不得硬写一个固定日期**（曾因硬写导致多条日期错误）。
...
## 测试闭环流程（always on · 每次 bug 修复或新增需求后必走）

**每一次**处理完 bug 修复或新增需求（把该次要求的所有事项都做完）后，**必须**走完下面这套「生成用例 → 真实环境执行 → 修复 → 再生成 → 再执行」的闭环，直到完全没有 bug：

1. **每个环节都启动一个全新的子 Agent**（不复用上一个子 Agent），各司其职：
   - **子 Agent（生成用例）**：为本次对话涉及的改动，生成**专业测试用例**，以 **Markdown 文件**形式输出（放 `docs/qa/`，含用例ID/优先级/前置/步骤/预期/类型，覆盖正向/边界/异常/回归/兼容）。
   - **子 Agent（执行用例）**：在**网页端真实环境**（优先线上 `look3.cn`，含主站 iframe 与独立页；必要时本地）**真实运行**这些用例（用浏览器实际点击/输入/观察，而非仅读代码），逐条判定通过/失败，产出结果报告（哪些用例失败、复现步骤、实际 vs 预期）。
2. **若发现问题**：
   - 启动**全新的子 Agent（修复）**修掉这些 bug 并部署；
   - 再启动**全新的子 Agent（重新生成用例）**；
   - 再启动**全新的子 Agent（重新执行）**在真实环境跑。
3. **如此循环**（生成→执行→修复→再生成→再执行……），**直至完全没有 bug**。
4. **最多循环 10 次**。若循环满 10 次仍有未解决的问题，则**终止流程**，并用**极简风格**向用户汇报（只说：还剩哪些问题未解决、已尝试几轮）。
5. 原则：**每个事项/环节 = 一个全新的子 Agent**；执行环节必须是**网页端真实环境跑通**，不能只做静态检查。
